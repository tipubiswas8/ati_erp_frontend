    ...
    <div :class="themeBlock" class="flex-1 w-full pt-14 p-4 space-y-8 relative min-h-screen">
        <div class="mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="progress-main">
                <div class="progress-container" :data-steps="steps.length" :style="{ '--current-step': currentStep }">
                    <div v-for="(step, index) in steps" :key="index" class="progress-item">
                        ...
                        }" class="step-number" v-on:click="currentDiv(index)">
                            {{ index + 1 }}
                        </div>
                        <span :class="{
                            'active-text': currentStep >= index + 1,
                            'inactive-text': currentStep < index + 1
                        }" class="step-label" v-on:click="currentDiv(index)">
                            {{ step }}
                        </span>
                    </div>
                </div>
            ...
            <div style="margin-left: 50px; margin-right: 50px; margin-bottom: 50px;">
                <div class="flex mt-8 pt-6 border-t border-gray-200">
                    <button v-if="currentStep > 1" @click="prevStep" type="button"
                        class="px-6 py-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-all duration-200 flex items-center">
...
import FamilyInformation from './FamilyInformation.vue';
import GuardianInformation from './GuardianInformation.vue';
import JobInformation from './JobInformation.vue';
import ProbationaryInformation from './ProbationaryInformation.vue';
import ContactInformation from './ContactInformation.vue';
import AddressInformation from './AddressInformation.vue';
import EducationInformation from './EducationInformation.vue';
import TrainingInformation from './TrainingInformation.vue';
import ProfessionalInformation from './ProfessionalInformation.vue';
import ExperienceInformation from './ExperienceInformation.vue';
import ReferenceInformation from './ReferenceInformation.vue';
import DocumentInformation from './DocumentInformation.vue';
import TerminationInformation from './TerminationInformation.vue';
