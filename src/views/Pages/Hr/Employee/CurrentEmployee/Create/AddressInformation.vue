    ...
    <form @submit.prevent="handleSubmit" class="p-8">
        ...
        <div class="space-y-6">
            <h2 class="text-2xl font-semibold text-gray-800 border-b-2 border-indigo-100 pb-3 text-center">
                Employee Address Information</h2>
...
const handleSubmit = () => {
    ...
    let selectdAddress: object;
    let textAddress: object;

    if (addressType.value === 'select') {
        selectdAddress = { ...formData.value, 'type': addressType.value };
        console.log('select', selectdAddress);
    } else {
        textAddress = { 'present_address': formData.value.present.present_address_text, 'permanent_address': formData.value.permanent.permanent_address_text, 'type': addressType.value };
    }
};
