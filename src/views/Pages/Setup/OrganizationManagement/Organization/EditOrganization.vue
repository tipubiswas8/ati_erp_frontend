...
const emit = defineEmits(["close", "organization-added"]);
...
async function submitForm() {
  try {
    const response = await globals.axios.post(globals.apiEndpoints.basePath + "/hr/organizations", form.value);
    ...
    if (response.data && response.data.status === true) {
      ...
      toast.error("Organization addition failed. Please try again!");
    }
  } catch (error) {
