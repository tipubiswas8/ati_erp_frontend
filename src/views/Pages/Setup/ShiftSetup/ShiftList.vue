<template>
    <div :class="themeBlock" class="flex-1 w-full pt-14 shadow-lg p-4 space-y-8 relative h-[100vh]">
        ...
        <div :class="themeClass" class="overflow-y-auto rounded-lg shadow-xl p-6"
            ...
            <table v-else :class="tableClass"
                ...
                <tbody :class="themeClass">
                    <template v-for="(shift, index) in shifts" :key="shift.shift_id">
                        <tr :class="getRowClass(index)">
                            ...
                            <td v-if="tableHeaders.columnNo5.visibility" class="p-4">{{
                                shift.shift_hours }}</td>
                            <td v-if="tableHeaders.columnNo6.visibility" class="p-4">{{
                                shift.user_define_shift_number }}</td>
                            <td v-if="tableHeaders.columnNo7.visibility" class="p-4 text-center w-12"
                                style="display: flex; gap: 6px;">
                                <button class="fa fa-eye" v-on:click="showShift(shift.id)"
                                    style="color: white; background-color: #3498db; border-radius: 50%; padding: 5px; border: none; cursor: pointer;"
                                    title="View"></button>
                                <router-link :to="`/setup/edit-shift/${shift.id}`"
                                    class="fa fa-pencil text-blue-600 hover:text-blue-800 transition-colors p-1 rounded hover:bg-blue-50 dark:hover:bg-gray-700 inline-block"
                                    title="Edit"
        ...
        <router-view @shift-created="handleShiftCreated" @shift-updated="handleShiftUpdated"></router-view>
    </div>
</template>
...
const routePath = '/setup/create-new-shift';
...
const tableHeaders = ref({
    ...
    columnNo2: { id: 2, key: 'shift_name', label: 'Shift Name', visibility: true, searchable: true },
    columnNo3: { id: 3, key: 'start_time', label: 'Start Time', visibility: true, searchable: true },
    columnNo4: { id: 4, key: 'end_time', label: 'End Time', visibility: true, searchable: true },
    columnNo5: { id: 5, key: 'shift_hours', label: 'Shift Hours', visibility: true, searchable: true },
    columnNo6: { id: 6, key: 'user_define_shift_number', label: 'User Defined Serial', visibility: true, searchable: true },
...
const showShift = (shiftId: string) => {
    router.push({
        name: 'ShowShift',
...
function handleShiftCreated(newShift) {
    shifts.value.unshift(newShift);
    toast.success("Shift created successfully!");
}

function handleShiftUpdated(updatedShift) {
    const index = shifts.value.findIndex(c => c.id === updatedShift.id);
    if (index !== -1) {
