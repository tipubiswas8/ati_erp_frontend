                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="bg-white p-6 rounded-lg shadow space-y-4 border border-gray-200">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex-1 w-full">
                                <a-select v-model:value="formData.hr_organization_id" show-search
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <input type="text" v-model="formData.shift_name"
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <a-time-picker v-model:value="formData.start_time" format="hh:mm A" class="flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2 focus:ring-indigo-500
                            focus:outline-none transition duration-200" />

                            <label class="w-40 text-sm font-medium text-gray-700 text-right">End Time </label>
                            <a-time-picker v-model:value="formData.end_time" format="hh:mm A" class="flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2 focus:ring-indigo-500
                            focus:outline-none transition duration-200" />
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-6 rounded-lg shadow space-y-4 border border-gray-200">
                        <div class="flex items-center gap-4">
                            ...
                            <input type="number" step="0.01" v-model="formData.shift_hours"
                                class="flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition duration-200" />
                        </div>
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">User Define Serial Number </label>
                            <input type="number" v-model="formData.user_define_shift_number"
                                class="flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition duration-200" />
                        </div>
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">Default Flag</label>
                            <select v-model="formData.default_shift_flag"
...
import { updateShift } from '@/api/setupApi';
...
const shiftIdParam = route.params?.id;
...
async function getShiftById(id: string) {
    const shift = await commonAxiosRequest.fetchDataById('su/shifts', id);
    try {
        if (shift.status && shift.data) {
            formData.value = {
                ...shift.data,
                start_time: dayjs(shift.data.start_time, 'hh:mm A'),
                end_time: dayjs(shift.data.end_time, 'hh:mm A')
            };
        }
    } catch (error) {
        console.log(error);
...
const handleSubmit = async () => {
    ...
    if (!formData.value.hr_organization_id) {
    ...
    try {
        const payload = {
            ...formData.value,
            // start_time: dayjs(formData.value.start_time).format('HH:mm'), // for 24h format
            // start_time: dayjs(formData.value.start_time)?.format('hh:mm A'), // for 12h AM/PM format
            end_time: dayjs(formData.value.end_time)?.format('hh:mm A'),
        };
        const update = await updateShift(formData.value.id, payload);
        console.log(update);

        if (update.success == true) {
            toast.success(update.message);
            router.push('/setup/shift-setup');
            emit('shift-created', update.data);
        }
    } catch (error) {
        console.log(error);
