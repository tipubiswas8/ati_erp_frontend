<template>
    ...
    <div :class="themeBlock" class="flex-1 w-full pt-14 shadow-lg p-4 space-y-8 relative h-[100vh]">
        ...
        <div :class="themeClass" class="overflow-y-auto rounded-lg shadow-xl p-6"
            ...
            <table v-else :class="tableClass"
                ...
                <tbody :class="themeClass">
                    <template v-for="(thana, index) in thanas" :key="thana.thana_id">
                        <tr :class="getRowClass(index)">
                            <td v-if="tableHeaders.columnNo1.visibility" class="p-4">{{ index + 1 }}</td>
                            <td v-if="tableHeaders.columnNo2.visibility" class="p-4"> {{
                                thana.district?.division?.country?.country_name || '' }}</td>
                            <td v-if="tableHeaders.columnNo3.visibility" class="p-4">{{
                                thana.district?.division?.division_name
                                }}</td>
                            <td v-if="tableHeaders.columnNo4.visibility" class="p-4">{{ thana.district?.district_name
                                }}</td>
                            <td v-if="tableHeaders.columnNo5.visibility" class="p-4">{{ thana.thana_name }}</td>
                            <td v-if="tableHeaders.columnNo6.visibility" class="p-4">{{
                                thana.user_defined_serial_number }}</td>
                            <td v-if="tableHeaders.columnNo7.visibility" class="p-4 text-center w-12">
                                <router-link :to="`/setup/edit-thana/${thana.id}`"
        ...
        <router-view @thana-created="handleThanaCreated" @thana-updated="handleThanaUpdated"></router-view>
    </div>
</template>
...
import { fetchAllThanas } from '@/api/setupApi';
import AddressItemMenu from './../AddressSeutp.vue';

const store = useStore();
const toast = useToast();
const routePath = '/setup/create-new-thana';
...
async function loadThanaInfo() {
    try {
        isLoading.value = true;
        const response = await fetchAllThanas();
...
function handleThanaCreated(newThana) {
    thanas.value.unshift(newThana);
    toast.success("Thana created successfully!");
}

function handleThanaUpdated(updatedThana) {
    const index = thanas.value.findIndex(c => c.id === updatedThana.id);
    if (index !== -1) {
        thanas.value[index] = updatedThana;
    }
