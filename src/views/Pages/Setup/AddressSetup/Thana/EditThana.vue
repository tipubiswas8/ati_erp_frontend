...
import { fetchAllCountries, getDivisionsByCountryId, getDistrictsByDivisionId, fetchThanaById, fetchDistrictById, fetchAllDistricts, updateThana } from '@/api/setupApi';
import { useToast } from "vue-toastification";
const emit = defineEmits(['thana-updated']);
...
onMounted(async () => {
    getCountries();
    const thana = fetchThanaById(props['id']);
    const receivedData = {
        ...(await thana).data,
        'country_id': '43'
    }
...
const handleSubmit = async () => {
    ...
    try {
        const update = await updateThana(props['id'], freshData);
        if (update.success == true) {
            toast.success(update.message);
            router.push('/setup/thana-setup');
            emit('thana-updated', update.data);
        }
    } catch (error) {
        console.log(error);
