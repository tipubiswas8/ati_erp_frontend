<template>
    ...
    <div :class="themeBlock" class="flex-1 w-full pt-14 shadow-lg p-4 space-y-8 relative h-[100vh]">
        ...
        <div :class="themeClass" class="overflow-y-auto rounded-lg shadow-xl p-6"
            ...
            <table v-else :class="tableClass"
                ...
                <tbody :class="themeClass">
                    <template v-for="(country, index) in countries" :key="country.country_id">
                        <tr :class="getRowClass(index)">
                            <td v-if="tableHeaders.columnNo1.visibility" class="p-4">{{ index + 1 }}</td>
                            <td v-if="tableHeaders.columnNo2.visibility" class="p-4">{{ country.country_name }}</td>
                            <td v-if="tableHeaders.columnNo3.visibility" class="p-4">{{ country.country_code }}</td>
                            <td v-if="tableHeaders.columnNo4.visibility" class="p-4">{{ country.iso }}</td>
                            <td v-if="tableHeaders.columnNo5.visibility" class="p-4">{{ country.phone_code }}</td>
                            <td v-if="tableHeaders.columnNo6.visibility" class="p-4 text-center w-12">
                                <router-link :to="`/setup/edit-country/${country.id}`"
        ...
        <router-view @country-created="handleCountryCreated" @country-updated="handleCountryUpdated"></router-view>
    </div>
</template>
...
import { fetchAllCountries } from '@/api/setupApi';
import AddressItemMenu from './../AddressSeutp.vue';


const store = useStore();
const toast = useToast();
const routePath = '/setup/create-new-country';
...
const tableHeaders = ref({
    ...
    columnNo3: { id: 3, key: 'country_code', label: 'country Code', visibility: true, searchable: true },
    columnNo4: { id: 4, key: 'iso', label: 'ISO', visibility: true, searchable: true },
    columnNo5: { id: 5, key: 'phone_code', label: 'Phone Code', visibility: true, searchable: true },
    columnNo6: { id: 5, key: '', label: 'Edit', visibility: true, searchable: true }
});
