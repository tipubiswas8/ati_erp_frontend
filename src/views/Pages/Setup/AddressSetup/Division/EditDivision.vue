            ...
            <div class="p-6 bg-gray-50 rounded-xl shadow-lg mx-auto">
                <div class="grid grid-cols-1 md:grid-cols-1 gap-8">
                    <div class="bg-white p-6 rounded-lg shadow space-y-4 border border-gray-200">
                        ...
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">Division Name</label>
                            <input type="text" v-model="formData.division_name"
                        ...
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">Division Code</label>
                            <input type="text" v-model="formData.user_defined_division_code"
                                class="flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition duration-200" />
                        </div>
                        <div class="flex items-center gap-4">
...
import { updateDivision, fetchAllCountries, fetchDivisionById } from '@/api/setupApi';
import { useToast } from "vue-toastification";
const emit = defineEmits(['division-updated']);
...
onMounted(async () => {
    getCountries();
    const division = fetchDivisionById(props.id);
    formData.value = (await division).data;
});


const handleSubmit = async () => {
    try {
        const add = await updateDivision(props.id, formData.value);
        if (add.success == true) {
            toast.success(add.message);
            router.push('/setup/division-setup');
            emit('division-updated', add.data);
        }
    } catch (error) {
        console.log(error);
