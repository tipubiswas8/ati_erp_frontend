...
import { addDivision, fetchAllCountries } from '@/api/setupApi';
import { useToast } from "vue-toastification";
const emit = defineEmits(['division-created']);
...
const handleSubmit = async () => {
    try {
        const add = await addDivision(formData.value);
        if (add.success == true) {
            toast.success(add.message);
            router.push('/setup/division-setup');
            emit('division-created', add.data);
        }
    } catch (error) {
        console.log(error);
