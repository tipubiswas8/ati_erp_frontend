...
import { addDistrict, fetchAllCountries, getDivisionsByCountryId } from '@/api/setupApi';
import { useToast } from "vue-toastification";
const emit = defineEmits(['district-created']);
...
const handleSubmit = async () => {
    ...
    try {
        const add = await addDistrict(freshData);
        if (add.success == true) {
            toast.success(add.message);
            router.push('/setup/district-setup');
            emit('district-created', add.data);
        }
    } catch (error) {
        console.log(error);
