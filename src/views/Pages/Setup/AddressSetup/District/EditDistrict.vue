            ...
            <div class="p-6 bg-gray-50 rounded-xl shadow-lg mx-auto">
                <div class="grid grid-cols-1 md:grid-cols-1 gap-8">
                    <div class="bg-white p-6 rounded-lg shadow space-y-4 border border-gray-200">
                        ...
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">District Name</label>
                            <input type="text" v-model="formData.district_name"
                        ...
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">District Code</label>
                            <input type="text" v-model="formData.user_defined_district_code"
                                class="flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition duration-200" />
                        </div>
                        <div class="flex items-center gap-4">
...
import { updateDistrict, fetchAllCountries, getDivisionsByCountryId, fetchDistrictById } from '@/api/setupApi';
import { useToast } from "vue-toastification";
const emit = defineEmits(['district-updated']);
...
onMounted(async () => {
    getCountries();
    const district = fetchDistrictById(props.id);
    const receivedData = {
        ...(await district).data,
        'country_id': '43'
    }
...
const handleSubmit = async () => {
    const { country_id, ...freshData } = formData.value;
    try {
        const add = await updateDistrict(props.id, freshData);
        if (add.success == true) {
            toast.success(add.message);
            router.push('/setup/district-setup');
            emit('district-updated', add.data);
        }
    } catch (error) {
        console.log(error);
