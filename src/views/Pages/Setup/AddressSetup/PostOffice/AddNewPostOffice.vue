...
import { addPostOffice, fetchAllCountries, getDistrictsByDivisionId, getDivisionsByCountryId, getThanasByDistrictId, getUnionsByThanaId } from '@/api/setupApi';
import { useToast } from "vue-toastification";
const emit = defineEmits(['post-office-created']);
...
const handleSubmit = async () => {
    ...
    try {
        const add = await addPostOffice(freshData);
        if (add.success == true) {
            toast.success(add.message);
            router.push('/setup/post-office-setup');
            emit('post-office-created', add.data);
        }
    } catch (error) {
        console.log(error);
