                ...
                <div class="grid grid-cols-1 md:grid-cols-1 gap-8">
                    <div class="bg-white p-6 rounded-lg shadow space-y-4 border border-gray-200">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <select v-model="formData.district_id" v-on:change="getThanas(formData.district_id)"
                                class="flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2 focus:ring-indigo-500">
                                <option value="">Select</option>
                        ...
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">Thana</label>
                            <select v-model="formData.thana_id" v-on:change="getUnions(formData.thana_id)"
                                class="flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2 focus:ring-indigo-500">
                                <option value="">Select</option>
                        ...
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">Union</label>
                            <select v-model="formData.union_id"
                                class="flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2 focus:ring-indigo-500">
                                <option value="">Select</option>
                        ...
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">Post Office Name</label>
                            <input type="text" v-model="formData.post_office_name"
                        ...
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">Post Office Code</label>
                            <input type="text" v-model="formData.user_defined_post_office_code"
                                class="flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition duration-200" />
                        </div>
                        <div class="flex items-center gap-4">
...
import { fetchAllCountries, fetchAllDistricts, fetchAllThanas, fetchAllUnions, fetchPostOfficeById, fetchUnionById, getDistrictsByDivisionId, getDivisionsByCountryId, getThanasByDistrictId, getUnionsByThanaId, updatePostOffice } from '@/api/setupApi';
import { useToast } from "vue-toastification";
const emit = defineEmits(['post-office-updated']);
...
async function getThanas(districtId: string) {
    const dist = getThanasByDistrictId(districtId);
    thanas.value = (await dist).data;
}

async function getUnions(thanaId: string) {
    const union = getUnionsByThanaId(thanaId);
    unions.value = (await union).data;
}

onMounted(async () => {
    getCountries();
    const postOffice = fetchPostOfficeById(props['id']);
    const receivedData = {
        ...(await postOffice).data,
        'country_id': '43'
    }
    getDivisions(receivedData.country_id);
    districts.value = (await fetchAllDistricts()).data;
    thanas.value = (await fetchAllThanas()).data;
    unions.value = (await fetchAllUnions()).data;
    const union = (await fetchUnionById(receivedData.union_id)).data;
    receivedData.union_id = union.id;
    formData.value = receivedData;
})
const handleSubmit = async () => {
    const { country_id, division_id, district_id, thana_id, ...freshData } = formData.value;
    try {
        const update = await updatePostOffice(props['id'], freshData);
        if (update.success == true) {
            toast.success(update.message);
            router.push('/setup/post-office-setup');
            emit('post-office-updated', update.data);
        }
    } catch (error) {
        console.log(error);
