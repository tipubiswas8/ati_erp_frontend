    ...
    <div :class="themeBlock" class="flex-1 w-full pt-14 shadow-lg p-4 space-y-8 relative h-[100vh]">
        ...
        <div :class="themeClass" class="overflow-y-auto rounded-lg shadow-xl p-6"
            ...
            <router-link :to="routePath"
                ...
                aria-label="Add New Post Office">
        ...
        <router-view @postOffice-created="handlePostOfficeCreated"
            @postOffice-updated="handlePostOfficeUpdated"></router-view>
    </div>
</template>
...
import { fetchAllPostOffices } from '@/api/setupApi';
import AddressItemMenu from './../AddressSeutp.vue';

const store = useStore();
const toast = useToast();
const routePath = '/setup/create-new-post-office';
...
const tableHeaders = ref({
    ...
    columnNo5: { id: 5, key: 'thana_name', label: 'Thana Name', visibility: true, searchable: true },
    columnNo6: { id: 6, key: 'union_name', label: 'Union Name', visibility: true, searchable: true },
    columnNo7: { id: 7, key: 'post_office_name', label: 'Post Office Name', visibility: true, searchable: true },
    columnNo8: { id: 8, key: 'user_defined_serial_number', label: 'User Defined Code', visibility: true, searchable: true },
    columnNo9: { id: 9, key: '', label: 'Edit', visibility: true, searchable: true }
});

onMounted(async () => {
    await loadPostOfficeInfo();
});

async function loadPostOfficeInfo() {
    try {
        isLoading.value = true;
        const response = await fetchAllPostOffices();
...
function handlePostOfficeCreated(newPostOffice) {
    postOffices.value.unshift(newPostOffice);
    toast.success("Post Office created successfully!");
}

function handlePostOfficeUpdated(updatedPostOffice) {
    const index = postOffices.value.findIndex(c => c.id === updatedPostOffice.id);
    if (index !== -1) {
        postOffices.value[index] = updatedPostOffice;
    }
    toast.success("Post Office updated successfully!");
}
