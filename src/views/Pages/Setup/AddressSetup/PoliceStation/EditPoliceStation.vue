                    ...
                    <div class="bg-white p-6 rounded-lg shadow space-y-4 border border-gray-200">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <select v-model="formData.division_id" v-on:change="getDistricts(formData.division_id)"
                                class=" flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2
                        ...
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">Police Station Name</label>
                            <input type="text" v-model="formData.police_station_name"
                        ...
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">Police Station Code</label>
                            <input type="text" v-model="formData.user_defined_ps_code"
                                class="flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition duration-200" />
                        </div>
                        <div class="flex items-center gap-4">
...
import { fetchAllCountries, getDivisionsByCountryId, getDistrictsByDivisionId, fetchAllDistricts, fetchPoliceStationById, fetchDistrictById, updatePoliceStation } from '@/api/setupApi';
import { useToast } from "vue-toastification";

const emit = defineEmits(['police-station-updated']);
...
onMounted(async () => {
    ...
    getCountries();
    const policeStations = fetchPoliceStationById(props['id']);
    const receivedData = {
        ...(await policeStations).data,
        'country_id': '43'
    }
    getDivisions(receivedData.country_id);
    districts.value = (await fetchAllDistricts()).data;
    const district = (await fetchDistrictById(receivedData.district_id)).data;
    receivedData.district_id = district.id;
...
const handleSubmit = async () => {
    ...
    try {
        const update = await updatePoliceStation(props['id'], freshData);
        if (update.success == true) {
            toast.success(update.message);
            router.push('/setup/police-station-setup');
            emit('police-station-updated', update.data);
        }
    } catch (error) {
        console.log(error);
