...
import { addPoliceStation, fetchAllCountries, getDivisionsByCountryId, getDistrictsByDivisionId } from '@/api/setupApi';
import { useToast } from "vue-toastification";
const emit = defineEmits(['police-station-created']);
...
const handleSubmit = async () => {
    ...
    try {
        const add = await addPoliceStation(freshData);
        if (add.success == true) {
            toast.success(add.message);
            router.push('/setup/police-station-setup');
            emit('police-station-created', add.data);
        }
    } catch (error) {
        console.log(error);
