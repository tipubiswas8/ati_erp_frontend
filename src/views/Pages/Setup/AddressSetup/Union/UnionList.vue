<template>
    <AddressItemMenu />
     <div :class="themeBlock" class="flex-1 w-full pt-14 shadow-lg p-4 space-y-8 relative h-[100vh]">
            ...
            <table v-else :class="tableClass"
                ...
                <thead :class="headerClass">
                    <tr class="uppercase">
                        ...
                        <th v-if="tableHeaders.columnNo8.visibility" class="p-4 text-center w-12">Action</th>
                    </tr>
                </thead>
                <tbody :class="themeClass">

                    <template v-for="(union, index) in unions" :key="union.union_id">
                        <tr :class="getRowClass(index)">
                            <td v-if="tableHeaders.columnNo1.visibility" class="p-4">{{ index + 1 }}</td>
                            <td v-if="tableHeaders.columnNo2.visibility" class="p-4"> {{
                                union.thana?.district?.division?.country?.country_name || '' }}</td>
                            <td v-if="tableHeaders.columnNo3.visibility" class="p-4">{{
                                union.thana?.district?.division?.division_name
                                }}</td>
                            <td v-if="tableHeaders.columnNo4.visibility" class="p-4">{{
                                union.thana?.district?.district_name
                                }}</td>
                            <td v-if="tableHeaders.columnNo5.visibility" class="p-4">{{ union.thana?.thana_name }}</td>
                            <td v-if="tableHeaders.columnNo6.visibility" class="p-4">{{ union.union_name }}</td>
                            <td v-if="tableHeaders.columnNo7.visibility" class="p-4">{{
                                union.user_defined_serial_number }}</td>
                            <td v-if="tableHeaders.columnNo8.visibility" class="p-4 text-center w-12">
                                <router-link :to="`/setup/edit-union/${union.id}`"
        ...
        <router-view @union-created="handleUnionCreated" @union-updated="handleUnionUpdated"></router-view>
        </div>
</template>
...
import { fetchAllUnions } from '@/api/setupApi';
import AddressItemMenu from './../AddressSeutp.vue';

const store = useStore();
const toast = useToast();
const routePath = '/setup/create-new-union';
...
const tableHeaders = ref({
    ...
    columnNo7: { id: 7, key: 'user_defined_serial_number', label: 'User Defined Code', visibility: true, searchable: true },
    columnNo8: { id: 8, key: '', label: 'Edit', visibility: true, searchable: true }
});
...
async function loadUnionInfo() {
    try {
        isLoading.value = true;
        const response = await fetchAllUnions();
        unions.value = response.data;
        console.log(unions.value)
    } catch (error) {
...
function handleUnionCreated(newUnion) {
    unions.value.unshift(newUnion);
    toast.success("Union created successfully!");
}

function handleUnionUpdated(updatedUnion) {
    const index = unions.value.findIndex(c => c.id === updatedUnion.id);
    if (index !== -1) {
        unions.value[index] = updatedUnion;
    }
