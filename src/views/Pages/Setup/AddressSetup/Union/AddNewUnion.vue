...
import { addUnion, fetchAllCountries, getDistrictsByDivisionId, getDivisionsByCountryId, getThanasByDistrictId } from '@/api/setupApi';
import { useToast } from "vue-toastification";
const emit = defineEmits(['union-created']);
...
const handleSubmit = async () => {
    ...
    try {
        const add = await addUnion(freshData);
        if (add.success == true) {
            toast.success(add.message);
            router.push('/setup/union-setup');
            emit('union-created', add.data);
        }
    } catch (error) {
        console.log(error);
