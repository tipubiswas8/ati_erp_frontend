                ...
                <div class="grid grid-cols-1 md:grid-cols-1 gap-8">
                    <div class="bg-white p-6 rounded-lg shadow space-y-4 border border-gray-200">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <select v-model="formData.thana_id"
                                class="flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2 focus:ring-indigo-500">
                                <option value="">Select</option>
                        ...
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">Union Name</label>
                            <input type="text" v-model="formData.union_name"
                        ...
                        <div class="flex items-center gap-4">
                            <label class="w-40 text-sm font-medium text-gray-700">Union Code</label>
                            <input type="text" v-model="formData.user_defined_union_code"
                                class="flex-1 px-4 py-2 border rounded-md border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition duration-200" />
                        </div>
                        <div class="flex items-center gap-4">
...
import { fetchAllCountries, fetchAllDistricts, fetchAllThanas, fetchThanaById, fetchUnionById, getDistrictsByDivisionId, getDivisionsByCountryId, getThanasByDistrictId, updateUnion } from '@/api/setupApi';
import { useToast } from "vue-toastification";

const emit = defineEmits(['union-updated']);
...
onMounted(async () => {
    getCountries();
    const union = fetchUnionById(props['id']);
    const receivedData = {
        ...(await union).data,
        'country_id': '43'
    }
    ...
    const thana = (await fetchThanaById(receivedData.thana_id)).data;
    receivedData.thana_id = thana.id;
    formData.value = receivedData;
})

const handleSubmit = async () => {
    const { country_id, division_id, district_id, ...freshData } = formData.value;
    try {
        const update = await updateUnion(props['id'], freshData);
        if (update.success == true) {
            toast.success(update.message);
            router.push('/setup/union-setup');
            emit('union-updated', update.data);
        }
    } catch (error) {
        console.log(error);
