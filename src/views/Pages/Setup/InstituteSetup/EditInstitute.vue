...
import { fetchInstituteById, updateInstitute } from '@/api/setupApi';
...
onMounted(async () => {
    const institute = await fetchInstituteById(props.id);
    formData.value.instituteInfo = institute.data;
})

const handleSubmit = async () => {
    try {
        const add = await updateInstitute(props.id, formData.value.instituteInfo)
        if (add.success == true) {
