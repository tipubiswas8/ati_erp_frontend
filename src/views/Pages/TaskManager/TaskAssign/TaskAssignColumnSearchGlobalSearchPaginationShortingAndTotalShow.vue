  ...
  <div :class="themeBlock" class="flex-1 w-full pt-14 shadow-lg p-4 space-y-8 relative h-[100vh]">
    ...
    <div :class="themeClass" class="overflow-hidden rounded-lg shadow-xl p-6">
      ...
      <div class="flex justify-between items-center mt-6 flex-wrap gap-4" style="display: none;">
        ...
        <div></div>
        <!-- Visibility -->
        <!-- Add this near your other controls -->
        <div class="flex items-center space-x-2">
          <button @click="toggleVisibilityMenu" class="p-2 rounded-lg bg-gray-200 hover:bg-gray-300">
            Columns Visibility
          </button>

          <div v-if="showVisibilityMenu" class="absolute mt-10 bg-white shadow-lg rounded-lg p-2 z-10">
            <div v-for="(isVisible, column) in visibleColumns" :key="column"
              class="flex items-center p-2 hover:bg-gray-100">
              <input type="checkbox" :id="`col-${column}`" v-model="visibleColumns[column]" class="mr-2">
              <label :for="`col-${column}`">{{ formatColumnName(column) }}</label>
            </div>
          </div>
        </div>
      ...
      <table :class="tableClass"
        ...
        <thead :class="headerClass">
          <tr class="text-left text-sm font-semibold uppercase">
            <th v-if="visibleColumns.task_id" class="p-4 text-center w-12"></th>
            <th v-if="visibleColumns.task_id" class="p-4 text-center">
              <div class="flex items-center justify-center">
                <input type="text" placeholder="Sl"
                  class="p-3 rounded-lg bg-gray-100 border-gray-300 w-12 focus:ring-2 focus:ring-blue-500 transition-all duration-300" />
            ...
            <th v-if="visibleColumns.project_name" class="p-4 text-center">
            ...
            <th v-if="visibleColumns.department_name" class="p-4 text-center">
              <div class="flex items-center">
            ...
            <th v-if="visibleColumns.task_title" class="p-4 text-center">
              <div class="flex items-center">
            ...
            <th v-if="visibleColumns.task_description" class="p-4 text-center">
              <div class="flex items-center">
            ...
            <th v-if="visibleColumns.attachment" class="p-4 text-center"></th>
          </tr>
        </thead>
        ...
        <tbody :class="themeClass">
          <template v-for="(task, index) in sortedAndPaginatedTasks" :key="task.task_id">
            <tr :class="getRowClass(index)">
              <td v-if="visibleColumns.task_id" class="p-4 text-center">
              ...
              <td v-if="visibleColumns.task_id" class="p-4"><span class="text-blue-600">{{ task.task_id }}</span></td>
              <td v-if="visibleColumns.project_name" class="p-4"><span class="text-blue-600">{{ task.project_name
                  }}</span></td>
              <td v-if="visibleColumns.department_name" class="p-4"><span class="text-blue-600">{{ task.department_name
                  }}</span></td>
              <td v-if="visibleColumns.task_title" class="p-4"><span class="text-blue-600">{{ task.task_title }}</span>
              </td>
              <td v-if="visibleColumns.task_description" class="p-4"><span class="text-blue-600">{{
                task.task_description }}</span></td>
              <td v-if="visibleColumns.attachment" class="p-4">
